...
from rorapp.helpers.civil_war import CIVIL_WAR_LOCATION
...
class PayRansomAction(ActionBase):
    ...
    def execute(
        ...
        captive.captor = None
        # Captives return to Rome when ransomed (1.10.71), but a rebel has left it (1.11.32)
        captive.location = CIVIL_WAR_LOCATION if captive.rebel else "Rome"
        captive.save()

        destination = "rejoined the revolt" if captive.rebel else "returned to Rome"
        Log.create_object(
            game_id, f"{captive.display_name} was ransomed for {ransom}T and {destination}."
        )
        set_hrao(game_id)
